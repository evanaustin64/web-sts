...
const contactLinks: ContactLink[] = [
  ...
  },
  {
    type: 'instagram',
    name: 'DM Instagram',
    href: 'https://www.instagram.com/officialpageyozuri/',
    icon: (
...
export default function FloatingContactButton() {
  ...
  return (
    <>
      {/* ===== KANAN BAWAH: Flowise BubbleChat ===== */}
      <BubbleChat
        ...
        theme={{
          ...
          chatWindow: { 
            title: 'CS PT. Samudra — AI',
            welcomeMessage: 'Halo! Ada yang bisa kami bantu? 😊',
            backgroundColor: '#ffffff',
            titleBackgroundColor: '#1a56db', // ← warna header chat window
      ...
      {/* ===== KIRI BAWAH: Tombol WA, IG, Email ===== */}
      <div className="fixed bottom-8 left-8 z-50 flex flex-col items-center">
        {/* Menu Opsi */}
        <div
          className={`flex flex-col items-center space-y-3 transition-all duration-300 ease-in-out mb-4 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
            }`}
        >
          {contactLinks.map((link) => (
            ...
            >
              {/* Tooltip — muncul ke kanan */}
              <div
                className={`absolute left-16 bg-white text-gray-800 text-sm px-3 py-1 rounded-md shadow-lg whitespace-nowrap transition-all duration-300 ${hoveredIcon === link.type ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2 pointer-events-none'
                  }`}
              >
                {link.name}
                <div className="absolute right-full top-1/2 -translate-y-1/2 w-0 h-0 border-t-4 border-t-transparent border-b-4 border-b-transparent border-r-4 border-r-white"></div>
              </div>
              <a
                href={link.href}
                target="_blank"
        ...
        {/* Tombol Utama (Headset) */}
        <div
          className="relative flex items-center"
          onMouseEnter={() => setIsMainButtonHovered(true)}
          onMouseLeave={() => setIsMainButtonHovered(false)}
        >
          {/* Tooltip tombol utama — muncul ke kanan */}
          <div
            className={`absolute left-[72px] bg-white text-gray-800 text-lg px-3 py-1 rounded-md shadow-lg whitespace-nowrap transition-all duration-300 ${isMainButtonHovered ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2 pointer-events-none'
              }`}
          >
            {isOpen ? 'Tutup Menu' : 'Hubungi Kami'}
            <div className="absolute right-full top-1/2 -translate-y-1/2 w-0 h-0 border-t-4 border-t-transparent border-b-4 border-b-transparent border-r-4 border-r-white"></div>
          </div>
          ...
          >
            ...
            <div className="relative transition-transform duration-300" style={{ transform: isOpen ? 'rotate(45deg)' : 'rotate(0)' }}>
              {isOpen ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" fill="white" className="w-9 h-9">
